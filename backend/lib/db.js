...
/**
 * Phase A: analytics columns + triggers.
 * Adds first_outbound_at + closed_at to threads, backfills, and teaches the
 * existing response trigger to stamp first_outbound_at. Also installs a
 * tiny trigger that keeps closed_at in sync with status. Idempotent.
 * Mirrored in migrations/032_inbox_analytics_columns.sql.
 */
async function migrateInboxAnalyticsColumns(p) {
  await p.query(`
    ALTER TABLE threads
  ...
  // Backfill from ticket_responses.
  await p.query(`
    UPDATE threads th
  ...
  // Approximate closed_at for already-closed rows.
  await p.query(`
    UPDATE threads
  ...
  // Trigger to maintain closed_at going forward.
  await p.query(`
  ...
  await p.query(`DROP TRIGGER IF EXISTS trg_thread_closed_at ON threads`);
  await p.query(`
  ...
  // Extend refresh_thread_from_response to stamp first_outbound_at on
  // the first sent reply.
  await p.query(`
