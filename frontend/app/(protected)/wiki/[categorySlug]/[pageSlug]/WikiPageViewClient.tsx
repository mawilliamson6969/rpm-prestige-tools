...
export default function WikiPageViewClient({
  categorySlug,
  pageSlug,
}: {
  categorySlug: string;
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const dRes = await fetch(apiUrl(`/wiki/pages/${row.id}`), {
        headers: { ...authHeaders() },
        cache: "no-store",
      });
  ...
  const downloadAtt = async (id: number, filename: string) => {
    const res = await fetch(apiUrl(`/wiki/attachments/${id}`), { headers: { ...authHeaders() } });
    if (!res.ok) return;
  ...
  const onPin = async () => {
    ...
    setBusy(true);
    try {
      const res = await fetch(apiUrl(`/wiki/pages/${page.id}/pin`), {
        method: "PUT",
        headers: { ...authHeaders() },
      });
  ...
  const onDelete = async () => {
    ...
    setBusy(true);
    try {
      const res = await fetch(apiUrl(`/wiki/pages/${page.id}`), {
        method: "DELETE",
        headers: { ...authHeaders() },
      });
      if (res.ok) router.replace(`/wiki/${categorySlug}`);
    } finally {
      setBusy(false);
      setDeleteOpen(false);
    }
  ...
  const onUpload = async (e: ChangeEvent<HTMLInputElement>) => {
    ...
    fd.append("file", file);
    const res = await fetch(apiUrl(`/wiki/pages/${page.id}/attachments`), {
      method: "POST",
      headers: { ...authHeaders() },
      body: fd,
    });
  ...
  const onDeleteAtt = async (id: number) => {
    if (!page) return;
    const res = await fetch(apiUrl(`/wiki/attachments/${id}`), {
      method: "DELETE",
      headers: { ...authHeaders() },
    });
    if (res.ok) await load();
  ...
  return (
    <>
      <div className={shell.breadcrumb}>
        ...
        <Link href="/wiki">Wiki</Link>
        <span>/</span>
        <Link href={`/wiki/${categorySlug}`}>{page.category_name}</Link>
        <span>/</span>
      ...
      <div className={shell.readLayout}>
        ...
        <article className={shell.articleCol}>
          <div className={art.actions}>
            <Link href={`/wiki/${categorySlug}/${pageSlug}/edit`} className={shell.btnPrimary}>
              Edit
            </Link>
