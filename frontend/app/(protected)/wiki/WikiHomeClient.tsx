...
export default function WikiHomeClient() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      setLoadErr(e instanceof Error ? e.message : "Could not load wiki.");
    }
  }, [authHeaders, token]);
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const res = await fetch(apiUrl(`/wiki/search?q=${encodeURIComponent(debouncedQ)}`), {
          headers: { ...authHeaders() },
        });
        const body = await res.json().catch(() => ({}));
        if (!cancelled) {
  ...
  return (
    <>
            ...
            >
              {searching ? (
                ...
                hits.map((h) => (
                  <Link
                    key={h.id}
                    href={`/wiki/${h.categorySlug}/${h.slug}`}
      ...
      <div className={styles.toolbar} style={{ justifyContent: "center" }}>
        <Link href="/wiki/new" className={styles.btnPrimary}>
          New Page
        </Link>
        {isAdmin ? (
          <Link href="/wiki/manage" className={styles.btnSecondary}>
            Manage Categories
          </Link>
        ) : null}
      </div>
      ...
      <ul className={styles.recentList}>
        {recent.length === 0 ? (
          <li>
            <span style={{ display: "block", padding: "0.85rem 1rem", color: "#6a737b" }}>No pages yet.</span>
          </li>
        ) : (
          recent.map((p) => (
            <li key={p.id}>
              <Link href={`/wiki/${p.category_slug}/${p.slug}`}>
      ...
      <h2 className={styles.sectionTitle}>Departments ({totalPages} pages)</h2>
      <div className={styles.cardGrid}>
        {categories.map((c) => (
          <Link key={c.id} href={`/wiki/${c.slug}`} className={styles.deptCard}>
            ...
            <div className={styles.deptMeta}>{c.page_count} pages</div>
          </Link>
        ))}
      </div>
    </>
