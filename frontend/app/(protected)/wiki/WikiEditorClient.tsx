...
import ed from "./wiki-editor.module.css";
...
function draftKey(parts: string[]) {
  return `wiki_draft_${parts.join("_")}`;
}
...
export default function WikiEditorClient({
  ...
  const loadCategories = useCallback(async () => {
    try {
      const res = await fetch(apiUrl("/wiki/categories"), { headers: { ...authHeaders() } });
  ...
  const loadPage = useCallback(async () => {
    ...
    try {
      const lRes = await fetch(
        apiUrl(
          `/wiki/pages?categorySlug=${encodeURIComponent(categorySlug)}&pageSlug=${encodeURIComponent(pageSlug)}`
        ),
        { headers: { ...authHeaders() } }
      );
      ...
      if (!row) {
        setLoadErr("Page not found.");
        return;
      }
      const dRes = await fetch(apiUrl(`/wiki/pages/${row.id}`), { headers: { ...authHeaders() } });
  ...
  const onSave = async () => {
    ...
    try {
      if (mode === "new") {
        const res = await fetch(apiUrl("/wiki/pages"), {
          method: "POST",
        ...
        router.replace(`/wiki/${cat?.slug ?? "unknown"}/${p.slug}`);
        return;
      }
      if (!pageId) return;
      const res = await fetch(apiUrl(`/wiki/pages/${pageId}`), {
        method: "PUT",
        headers: { ...authHeaders(), "Content-Type": "application/json" },
        body: JSON.stringify({
          title: title.trim(),
      ...
      setChangeSummary("");
      router.push(`/wiki/${cat?.slug ?? categorySlug}/${p.slug}`);
    } catch (e) {
    ...
    <>
      <div className={shell.breadcrumb}>
        ...
        {catSlugForBreadcrumb ? (
          <>
            <span>/</span>
            <Link href={`/wiki/${catSlugForBreadcrumb}`}>{categories.find((c) => c.slug === catSlugForBreadcrumb)?.name ?? catSlugForBreadcrumb}</Link>
          </>
        ) : null}
        {mode === "edit" && pageSlug ? (
          <>
            <span>/</span>
            <Link href={`/wiki/${categorySlug}/${pageSlug}`}>{title || pageSlug}</Link>
            <span>/</span>
            <span>Edit</span>
          </>
        ) : (
      ...
      <h1 className={shell.pageTitle}>{mode === "new" ? "New page" : "Edit page"}</h1>

      <div className={ed.editorPage}>
        <div className={ed.fields}>
          <label className={ed.paneLabel} htmlFor="wiki-title">
            Title
          </label>
          <input
            id="wiki-title"
          ...
          <div className={ed.row}>
            <label className={ed.hint} htmlFor="wiki-cat">
              Department
            </label>
            <select
              id="wiki-cat"
          ...
          {mode === "edit" ? (
            <>
              <label className={ed.paneLabel} htmlFor="wiki-sum">
                What did you change?
              </label>
              <input
                id="wiki-sum"
            ...
            <p className={ed.hint}>Change summary is optional when creating a new page.</p>
          )}
        </div>
        ...
        <p className={ed.hint} style={{ marginTop: "0.75rem" }}>
          Draft auto-saves locally every 30 seconds (restore after a crash on new pages).
        </p>
      </div>
