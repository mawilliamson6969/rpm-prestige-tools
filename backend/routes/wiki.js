...
const wikiUploadRoot = path.join(__dirname, "..", "uploads", "wiki");
...
async function uniqueCategorySlug(pool, base, excludeId = null) {
  ...
  for (;;) {
    const q = excludeId
      ? `SELECT id FROM wiki_categories WHERE slug = $1 AND id <> $2`
      : `SELECT id FROM wiki_categories WHERE slug = $1`;
...
async function uniquePageSlug(pool, categoryId, base, excludeId = null) {
  ...
  for (;;) {
    const q = excludeId
      ? `SELECT id FROM wiki_pages WHERE category_id = $1 AND slug = $2 AND id <> $3`
      : `SELECT id FROM wiki_pages WHERE category_id = $1 AND slug = $2`;
...
function wikiPageUploadDir(pageId) {
  const dir = path.join(wikiUploadRoot, String(pageId));
...
export function wikiUploadMiddleware(req, res, next) {
  wikiUpload.single("file")(req, res, (err) => {
    if (err) {
...
export async function getWikiCategories(req, res) {
  try {
    const p = getPool();
    ...
    const { rows: tc } = await p.query(`SELECT COUNT(*)::int AS c FROM wiki_pages`);
    const { rows: recent } = await p.query(
      `SELECT w.id, w.title, w.slug, w.updated_at, w.status,
       ...
       FROM wiki_pages w
       JOIN wiki_categories c ON c.id = w.category_id
    ...
    console.error(e);
    res.status(500).json({ error: "Could not load wiki categories." });
  }
}

export async function postWikiCategory(req, res) {
  ...
  try {
    ...
    const { rows } = await p.query(
      `INSERT INTO wiki_categories (name, slug, description, icon, created_by, display_order)
       VALUES ($1, $2, $3, $4, $5,
         (SELECT COALESCE(MAX(display_order), -1) + 1 FROM wiki_categories))
...
export async function putWikiCategory(req, res) {
  const id = Number(req.params.id);
  if (!Number.isFinite(id) || id <= 0) {
...
export async function deleteWikiCategory(req, res) {
  const id = Number(req.params.id);
  if (!Number.isFinite(id) || id <= 0) {
    res.status(400).json({ error: "Invalid category id." });
    return;
  ...
  try {
    const p = getPool();
    const { rows: cnt } = await p.query(`SELECT COUNT(*)::int AS c FROM wiki_pages WHERE category_id = $1`, [id]);
    if ((cnt[0]?.c ?? 0) > 0) {
      res.status(400).json({ error: "Category still has pages; delete or move them first." });
      return;
    }
    const { rowCount } = await p.query(`DELETE FROM wiki_categories WHERE id = $1`, [id]);
    if (!rowCount) {
      res.status(404).json({ error: "Category not found." });
      return;
    }
...
export async function getWikiPages(req, res) {
  ...
  try {
    ...
    if (!catId && categorySlug) {
      const { rows: cr } = await p.query(`SELECT id FROM wiki_categories WHERE slug = $1`, [categorySlug]);
      catId = cr[0]?.id ?? null;
    }
    const params = [];
    const where = [];
    ...
    console.error(e);
    res.status(500).json({ error: "Could not load wiki pages." });
  }
}

export async function getWikiPage(req, res) {
  const id = Number(req.params.id);
  if (!Number.isFinite(id) || id <= 0) {
  ...
  try {
    ...
    const { rows } = await p.query(
      `SELECT w.*, c.slug AS category_slug, c.name AS category_name,
              cu.display_name AS created_by_name,
              eu.display_name AS last_edited_by_name,
              (SELECT MAX(version_number)::int FROM wiki_page_versions v WHERE v.page_id = w.id) AS current_version
       FROM wiki_pages w
       JOIN wiki_categories c ON c.id = w.category_id
       LEFT JOIN users cu ON cu.id = w.created_by
       LEFT JOIN users eu ON eu.id = w.last_edited_by
...
export async function postWikiPage(req, res) {
  ...
  try {
    const p = getPool();
    const { rows: cat } = await p.query(`SELECT id FROM wiki_categories WHERE id = $1`, [categoryId]);
    if (!cat.length) {
    ...
    const { rows } = await p.query(
      `INSERT INTO wiki_pages (
       ...
       VALUES (
         $1, $2, $3, $4, $5,
         $6, $6,
         (SELECT COALESCE(MAX(display_order), -1) + 1 FROM wiki_pages WHERE category_id = $1)
       )
       RETURNING *`,
    ...
    const page = rows[0];
    await p.query(
      `INSERT INTO wiki_page_versions (page_id, version_number, title, content_markdown, change_summary, edited_by)
       VALUES ($1, 1, $2, $3, $4, $5)`,
...
export async function putWikiPage(req, res) {
  const id = Number(req.params.id);
  if (!Number.isFinite(id) || id <= 0) {
    res.status(400).json({ error: "Invalid page id." });
    return;
  ...
  try {
    ...
    const { rows: cur } = await p.query(
      `SELECT id, category_id, title, slug, content_markdown, status FROM wiki_pages WHERE id = $1`,
      [id]
    );
    if (!cur.length) {
      res.status(404).json({ error: "Page not found." });
    ...
    const { rows: vmax } = await p.query(
      `SELECT COALESCE(MAX(version_number), 0)::int AS m FROM wiki_page_versions WHERE page_id = $1`,
      [id]
    );
...
export async function deleteWikiPage(req, res) {
  const id = Number(req.params.id);
  if (!Number.isFinite(id) || id <= 0) {
    res.status(400).json({ error: "Invalid page id." });
    return;
  ...
  try {
    const p = getPool();
    const { rows } = await p.query(`SELECT id, created_by FROM wiki_pages WHERE id = $1`, [id]);
    if (!rows.length) {
      res.status(404).json({ error: "Page not found." });
      return;
    }
    ...
    const dir = path.join(wikiUploadRoot, String(id));
    await p.query(`DELETE FROM wiki_pages WHERE id = $1`, [id]);
...
export async function putWikiPagePin(req, res) {
  const id = Number(req.params.id);
  if (!Number.isFinite(id) || id <= 0) {
    res.status(400).json({ error: "Invalid page id." });
    return;
  ...
  try {
    const p = getPool();
    const { rows } = await p.query(
      `UPDATE wiki_pages SET is_pinned = NOT COALESCE(is_pinned, false), updated_at = NOW()
       WHERE id = $1
...
export async function putWikiPageReorder(req, res) {
  const id = Number(req.params.id);
  ...
  try {
    const p = getPool();
    const { rows } = await p.query(
      `UPDATE wiki_pages SET display_order = $1, updated_at = NOW() WHERE id = $2 RETURNING id, display_order`,
      [displayOrder, id]
    );
    if (!rows.length) {
      res.status(404).json({ error: "Page not found." });
...
export async function getWikiPageVersions(req, res) {
  const id = Number(req.params.id);
  if (!Number.isFinite(id) || id <= 0) {
    res.status(400).json({ error: "Invalid page id." });
    return;
  ...
  try {
    ...
    const { rows: cur } = await p.query(
      `SELECT MAX(version_number)::int AS m FROM wiki_page_versions WHERE page_id = $1`,
      [id]
    );
...
export async function getWikiPageVersion(req, res) {
...
export async function postWikiRestoreVersion(req, res) {
  const pageId = Number(req.params.id);
  const versionId = Number(req.params.versionId);
  if (!Number.isFinite(pageId) || pageId <= 0 || !Number.isFinite(versionId) || versionId <= 0) {
    res.status(400).json({ error: "Invalid id." });
  ...
  try {
    ...
    const { rows: vr } = await p.query(
      `SELECT version_number, title, content_markdown FROM wiki_page_versions WHERE id = $1 AND page_id = $2`,
      [versionId, pageId]
    );
    if (!vr.length) {
      res.status(404).json({ error: "Version not found." });
...
export async function postWikiAttachment(req, res) {
  const pageId = Number(req.params.id);
  ...
  try {
    const p = getPool();
    const { rows } = await p.query(`SELECT id FROM wiki_pages WHERE id = $1`, [pageId]);
    if (!rows.length) {
    ...
    const { rows: ins } = await p.query(
      `INSERT INTO wiki_attachments (page_id, filename, stored_filename, file_size_bytes, mime_type, uploaded_by)
       VALUES ($1, $2, $3, $4, $5, $6)
...
export async function getWikiAttachment(req, res) {
  const id = Number(req.params.id);
  if (!Number.isFinite(id) || id <= 0) {
  ...
  try {
    const p = getPool();
    const { rows } = await p.query(
      `SELECT a.*, w.id AS page_id FROM wiki_attachments a JOIN wiki_pages w ON w.id = a.page_id WHERE a.id = $1`,
      [id]
    );
    if (!rows.length) {
    ...
    const row = rows[0];
    const abs = path.join(wikiUploadRoot, String(row.page_id), row.stored_filename);
...
export async function deleteWikiAttachment(req, res) {
  const id = Number(req.params.id);
  if (!Number.isFinite(id) || id <= 0) {
    res.status(400).json({ error: "Invalid attachment id." });
    return;
  ...
  try {
    ...
    const { rows } = await p.query(
      `SELECT a.id, a.page_id, a.stored_filename, a.filename, a.uploaded_by
       FROM wiki_attachments a WHERE a.id = $1`,
      [id]
    );
    if (!rows.length) {
      res.status(404).json({ error: "Attachment not found." });
    ...
    const abs = path.join(wikiUploadRoot, String(row.page_id), row.stored_filename);
    await p.query(`DELETE FROM wiki_attachments WHERE id = $1`, [id]);
...
export async function getWikiSearch(req, res) {
