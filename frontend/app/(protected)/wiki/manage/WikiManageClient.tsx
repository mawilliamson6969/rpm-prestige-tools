...
export default function WikiManageClient() {
  ...
  const onSaveEdit = async (id: number) => {
    setBusy(true);
    try {
      const res = await fetch(apiUrl(`/wiki/categories/${id}`), {
        method: "PUT",
        headers: { ...authHeaders(), "Content-Type": "application/json" },
        body: JSON.stringify({
          name: editName.trim(),
  ...
  const onDelete = async (c: Category) => {
    if (c.page_count > 0) {
      alert("Delete all pages in this department first.");
      return;
    }
    if (!window.confirm(`Delete department “${c.name}”?`)) return;
    setBusy(true);
    try {
      const res = await fetch(apiUrl(`/wiki/categories/${c.id}`), {
        method: "DELETE",
        headers: { ...authHeaders() },
      });
      if (!res.ok) {
  ...
  return (
    <>
      ...
      <h1 className={shell.pageTitle}>Manage departments</h1>
              ...
              <div style={{ display: "flex", flexWrap: "wrap", justifyContent: "space-between", gap: "0.75rem" }}>
                <div>
                  ...
                  <div style={{ fontSize: "0.8rem", marginTop: "0.25rem" }}>
                    slug: <code>{c.slug}</code> · {c.page_count} pages
                  </div>
                </div>
      ...
      <p style={{ marginTop: "1.5rem" }}>
        <Link href="/wiki" className={shell.btnSecondary}>
          ← Back to wiki
        </Link>
      </p>
    </>
