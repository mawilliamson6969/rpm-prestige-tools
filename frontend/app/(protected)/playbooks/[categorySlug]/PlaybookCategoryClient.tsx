...
export default function PlaybookCategoryClient({
  categorySlug,
}: {
  categorySlug: string;
}) {
  ...
  const load = useCallback(async () => {
    ...
    try {
      const [cRes, pRes] = await Promise.all([
        fetch(apiUrl("/playbooks/categories"), { headers: { ...authHeaders() }, cache: "no-store" }),
        fetch(apiUrl(`/playbooks/pages?categorySlug=${encodeURIComponent(categorySlug)}`), {
          headers: { ...authHeaders() },
          cache: "no-store",
        }),
      ]);
  ...
  if (err) return <p style={{ color: "#b32317" }}>{err}</p>;
  if (!ready && !err) return <p style={{ color: "#6a737b" }}>Loading…</p>;
  if (ready && !category) return <p>Category not found.</p>;
  ...
  return (
    <>
      ...
      <div className={shell.toolbar}>
        ...
        <Link href={`/playbooks/new?category=${encodeURIComponent(category.slug)}`} className={shell.btnPrimary}>
          New Playbook in {category.name}
        </Link>
      </div>
      ...
      <div className={shell.toolbar} style={{ marginBottom: "1rem" }}>
        <input
          ...
          aria-label="Search playbooks in category"
        />
      ...
      {pinned.length > 0 ? (
        <>
          ...
          <ul className={shell.recentList} style={{ marginBottom: "1.5rem" }}>
            {pinned.map((p) => (
              <li key={p.id}>
                <Link href={`/playbooks/${category.slug}/${p.slug}`}>
      ...
      <h2 className={shell.sectionTitle}>All playbooks</h2>
      <ul className={shell.recentList}>
        {rest.length === 0 ? (
          <li>
            <span style={{ display: "block", padding: "0.85rem 1rem", color: "#6a737b" }}>No playbooks in this view.</span>
          </li>
        ) : (
          rest.map((p) => (
            <li key={p.id}>
